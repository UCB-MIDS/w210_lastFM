copy (select a.userid ,a.gender genderid, CASE WHEN a.gender = 1 then 'm' WHEN a.gender = 0 then 'f' ELSE 'NA' END gender ,
 ...
 --where a.session_start < '03/31/2009' 
   ---and a.userid in (select distinct a.userid 
                      ---from user_session_aggregated a
   ...
   --and regexp_replace(a.userid, 'user_', '')::integer < 101
   --and exists (select 1 from user_session_aggregated where userid = a.userid and session_start < '03/31/2009')
order by a.userid, a.session_start) to 'C:\Users\jayashree.raman\Documents\Learning\MIDS\capstone\datasets\usersessions-with-genre.csv' DELIMITER '|' CSV HEADER;
