copy (select a.userid ,COALESCE(a.gender, 'NA') gender ,COALESCE(CAST(a.age AS char(2)), 'NA') age, COALESCE(a.country, 'NA') country, a.session_start::date startdate, 
 ...
 where a.session_start > '04/01/2009' 
   and exists (select 1 from user_session_aggregated where userid = a.userid and session_start < '03/31/2009')
order by a.userid, a.session_start) to 'C:\Users\jayashree.raman\Documents\Learning\MIDS\capstone\lastfm-dataset-1K\usersessions-with-char-sec-test.csv' DELIMITER '|' CSV HEADER;
