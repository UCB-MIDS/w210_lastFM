copy (select a.userid ,a.gender genderid, CASE WHEN a.gender = 1 then 'm' WHEN a.gender = 0 then 'f' ELSE 'NA' END gender ,
 ...
 LEFT OUTER JOIN country_holidays ch ON a.session_start::date = ch.hol_dt AND (lower(b.countryname) in (ch.country, ch.raw_country) OR lower(b.countryname) like ch.country||'%')
  where a.session_start < '03/31/2009' 
    ...
    and exists (select 1 from user_session_aggregated where userid = a.userid and session_start < '03/31/2009')
order by a.userid, a.session_start) to 'C:\Users\jayashree.raman\Documents\Learning\MIDS\capstone\datasets\usersessions-with-genre-train.csv' DELIMITER '|' CSV HEADER;
